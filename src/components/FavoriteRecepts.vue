...
<template>
  ...
  <div>
    ...
    <div v-for="recipe in favoriteRecipes" :key="recipe.id">
      <BCard class="favorite-card" @mouseover="enlargeImage" @mouseout="resetImage">
        <div class="card-content">
          <!-- Visar receptets titel -->
          <h3>{{ recipe.title }}</h3>
          <!-- Visar receptets bild -->
          <img :src="`src/assets/${recipe.image}`" alt="Recipe Image" class="recipe-image">
          <!-- Knapp för att ta bort receptet från favoriter -->
          <button @click="removeFromFavorites(recipe.id)" class="remove-button"> Ta bort recept </button>
        </div>
      </BCard>
    </div>
  </div>
